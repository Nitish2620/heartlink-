            ...
            >
                ...
                {hasContrastWarning && (
                  ...
                  >
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2.5 shadow-sm">
                      ...
                      <div>
                        <h4 className="text-[11px] font-bold text-amber-900 leading-tight">Low Contrast Warning</h4>
                        <p className="text-[10px] text-amber-700/90 mt-0.5 leading-snug">
                          The current Text Color might be hard to read against the Canvas Background. Try adjusting one of them for better accessibility.
                        </p>
                      </div>
                    </div>
                  </motion.div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2.5">
                   <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Instant Presets</span>
                   <button
                     type="button"
                     onClick={() => {
                        // Very simple randomize logic just for fun MNC business feature
                        const randomColor = () => '#' + Math.floor(Math.random()*16777215).toString(16).padStart(6, '0');
                        const newColors = { ...customColors };
                        for(let key in newColors) {
                           if (Math.random() > 0.3) (newColors as any)[key] = randomColor();
                        }
                    ...
                    >
                      ...
                      <div className={\`flex items-center gap-2 p-1.5 rounded-xl border shadow-xs transition-colors \${isHovered ? 'bg-white border-purple-200' : 'bg-slate-50 border-slate-200 group-hover:border-slate-300 group-hover:bg-white'}\`}>
                        ...
                        <input
                          ...
                          value={currentColor}
                          onChange={(e) => {
                             // Let them type freely. Validation will naturally fail in CSS if invalid.
                             // A true MNC app would buffer invalid strings and only apply valid ones, but this handles it gracefully.
                             setCustomColors(prev => ({ ...prev, [slot.key]: e.target.value }));
                             setUiTheme('custom');
                          }}
...
console.log('Successfully updated drawer UI logic to MNC grade');
